...
struct BlockFmhaPipelineQRKSVSFp8
{
               ...
               void* smem_ptr) const
    {
        ...
        //support for kK0=kK0BlockLength
	//fetch Kblocks in cache-line size(s)
	...
	//current bottleneck is softmax
	//softmax roughly takes about 1700 cycles
	//GEMM0+GEMM1 takes about 2048 cycle
	//with 2 WG/CU and software pipelining,
	//softmax is overlapped with GEMM0 (other WG) 1024/1700
	//effectively efficiency is 2048/(2048+676) 75% 
	//
	//softmax improvements 
	//1. replace fma with pk_fma
        ...
        do
        {
            ...
            raise_priority();		
            {                                                           // tail
	        if constexpr (k0_loops >= 2)                            // k0_loops<2
	        {		
                    block_sync_lds();
                    gemm_0(s_acc,
                           get_slice_tile(q_tile,
	    ...
	    //prefetch Vtile
            ...
            __builtin_amdgcn_sched_barrier(
                    0); // prevent from re-ordering global loads any further
							      //
            ...
            sweep_tile_span(p_spans[Number<0>{}], [&](auto idx0) {
                ...
                sweep_tile_span(p_spans[Number<1>{}], [&](auto idx1) {
                    ...
                    if constexpr(kHasBias)
                    {
		        //Replace this pk_fma_f32(i_0,i_1), v_exp_f32(i_0) v_exp_f32(i_1)
            ...
            __builtin_amdgcn_sched_barrier(
                    0); // prevent from  scheduling ds_stores  too early
            ...
            // l{j}, Oacc{j}
	    // FIXME: This is old scaling of oacc{j} with rowsum{j}
	    // we need to remove most of the code and scale oacc{i} with l{1} first rowsum

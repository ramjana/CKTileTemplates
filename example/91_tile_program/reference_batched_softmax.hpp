...
template <typename ADataType, typename BDataType, typename LSEDataType>
void reference_batched_softmax_with_lse(const Tensor<ADataType>& a_b_m_n,
                                        Tensor<BDataType>& b_b_m_n,
                                        Tensor<LSEDataType> lse_b_m)
{
    const int N = a_b_m_n.mDesc.GetLengths()[2];
    ...
    auto f = [&](auto batch, auto m) {
        for(int n = 0; n < N; ++n)
        {
            ...
            b_b_m_n(batch, m, n) = ck::math::exp(v_a - lse_b_m(batch, m));
        }
    };
